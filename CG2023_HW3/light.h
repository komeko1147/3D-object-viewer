...
public:
	...
	glm::vec3 GetSpotD() const { return direction; }
	float GetSpotCutoff() const { return cutoff; }
	float GetSpotTotalwidth() const { return totalwidth; }
	// -------------------------------------------------------
...
public:
	...
	void MoveLeft(const float moveSpeed) { direction += moveSpeed * glm::vec3(0.1f, 0.0f, 0.0f); }
	void MoveRight(const float moveSpeed) { direction += moveSpeed * glm::vec3(-0.1f, 0.0f, 0.0f); }
	void MoveUp(const float moveSpeed) { direction += moveSpeed * glm::vec3(0.0f, -0.1f, 0.0f); }
	void MoveDown(const float moveSpeed) { direction += moveSpeed * glm::vec3(0.0f, 0.1f, 0.0f); }
